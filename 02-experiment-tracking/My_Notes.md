...
- In "set_tracking_uri" the prefix before the path tells mlflow the type of storage we want to use: file/database. Should use 3 forward slashes after the storage type for an RELATIVE PATH: "sqlite:///RELATIVE PATH", however if using an absolute path, will need 4 slashses: "sqlite:////ABSOLUTE PATH".
